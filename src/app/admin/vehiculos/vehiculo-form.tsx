...
export function VehiculoForm({
  ...
  async function handleFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      const { data } = supabase.storage.from("vehiculos").getPublicUrl(nombre);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Pasar las URLs de las imágenes como campo oculto
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        >
          {uploading ? (
            <><Loader2 size={16} className="animate-spin" /> Subiendo fotos...</>
          ) : (
